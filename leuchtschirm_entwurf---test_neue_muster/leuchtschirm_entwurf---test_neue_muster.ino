...
SimplePatternList gPatterns = {RainbowColors_swoosh, RainbowColors_fade, RainbowStripeColors_fade, OceanColors_fade, LavaColors_fade, ForestColors_fade, //White_fade,
...
uint8_t swoosh_firsttime=1;   //Globaler Wert, der angibt, ob swoosh das erste mal ausgefuert wurde
...
void bpm()
{
  // colored stripes pulsing at a defined Beats-Per-Minute (h)
  CRGBPalette16 palette = PartyColors_p;
  uint8_t beat = beatsin8( blink_rate, 64, 255);
  for( int i = 0; i < NUM_LEDS_PER_STRIP; i++) { //9948
    leds[0][i] = ColorFromPalette(palette, gHue+(i*2), beat+(i*10));
  }
  for(int i=1;i<NUM_STRIPS;i++){                                 //Parallel fuer jeden Arm gleich
    memcpy(&leds[i], &leds[0], NUM_LEDS_PER_STRIP *sizeof(CRGB) );
  }
...
//-------------------------------------------------------
void PaletteColors_swoosh(CRGBPalette16 currentPalette)
{
  static unsigned long starttime,old_starttime; //in milli seconds
  static unsigned int rate; //bpm
  int timenow=millis();
  ...
  if (trigger && swoosh_firsttime){ 
    on=1;
    starttime=timenow;
    old_starttime=timenow;
    swoosh_firsttime=0;
  }
  else if (trigger){
    rate= int(60000/float(old_starttime-timenow));
    if (rate < 1){
      rate=60;
    }
    starttime=timenow;
  ...
  else if (on && (starttime-timenow > 60000/float(rate))){
    on=0;
    starttime=timenow;
  }

  uint8_t beat = beatsin8( rate, 64, 255,starttime);
  for( int i = 0; i < NUM_LEDS_PER_STRIP; i++) { //9948
    leds[0][i] = ColorFromPalette(currentPalette, gHue+(i*2), beat+(i*10));
  }
  for(int i=1;i<NUM_STRIPS;i++){                                 //Parallel fuer jeden Arm gleich
    memcpy(&leds[i], &leds[0], NUM_LEDS_PER_STRIP *sizeof(CRGB) );
  }
...
void RainbowColors_swoosh() 
{
  PaletteColors_swoosh(RainbowColors_p);
}
