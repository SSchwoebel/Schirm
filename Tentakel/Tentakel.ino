...
void setup() {
  ...
  //set interrupt pin for Brightness Switch
  pinMode(brightness_switch_interrupt_pin,INPUT_PULLUP);
  attachInterrupt(brightness_switch_interrupt_pin-2,nextBrightnessTrigger,LOW);  //brightness_switch_interrupt_pin-2 is a dirty workarround
...
void nextBrightnessSwitch()
{
   static unsigned long starttime;
   int deadtime=500;
   
   if (brightnessTrigger && brightnessSwitchPressed==0){
      starttime=millis();
      BRIGHTNESS=(BRIGHTNESS+BRIGHTNESS_INC)%256;
      FastLED.setBrightness(BRIGHTNESS);
      brightnessTrigger = 0;
      brightnessSwitchPressed = 1;
    }
    if((millis()-starttime>deadtime) && brightnessSwitchPressed==1){
      brightnessSwitchPressed = 0;
      brightnessTrigger = 0;
    } 
}
  
void nextBrightnessTrigger()
{
  brightnessTrigger=1;  
}
